...
CREATE TABLE deployments (
    ...
    schedule_type TEXT NOT NULL CHECK (schedule_type IN ('hourly', 'daily', 'weekly')),
...
COMMENT ON TABLE deployments IS 'Periodic Deployment System - scheduled script executions managed by APScheduler';
...
COMMENT ON COLUMN deployments.schedule_type IS 'Schedule frequency: hourly, daily, or weekly';
COMMENT ON COLUMN deployments.schedule_config IS 'JSON configuration for schedule (hour, minute, day)';
COMMENT ON COLUMN deployments.status IS 'Deployment status: active, paused, or stopped';
