...
AS $$
BEGIN
  -- Update node metrics for verifications (unchanged)
  ...
  -- Update edge metrics for actions - NOW WITH ACTION_SET_ID
  IF NEW.execution_type = 'action' AND NEW.edge_id IS NOT NULL AND NEW.action_set_id IS NOT NULL THEN
    INSERT INTO edge_metrics (edge_id, tree_id, team_id, action_set_id, total_executions, successful_executions, success_rate, avg_execution_time_ms)
    VALUES (
      NEW.edge_id,
      NEW.tree_id,
      NEW.team_id,
      NEW.action_set_id,  -- Include action_set_id
      1,
      CASE WHEN NEW.success THEN 1 ELSE 0 END,
      CASE WHEN NEW.success THEN 1.0 ELSE 0.0 END,
      COALESCE(NEW.execution_time_ms, 0)
    )
    ON CONFLICT (edge_id, tree_id, action_set_id) DO UPDATE SET  -- Updated conflict resolution
