...
- Routes `/server/*` → Backend Server (port 5109)
- Routes `/host1/*` → Backend Host 1 container (port 6109)
- Routes `/host2/*` → Backend Host 2 container (port 6110)
- Routes `/grafana/*` → Grafana (port 3000)
...
Host Nginx (port 443, SSL)
    ↓ rewrites /host1/stream/* → /host/stream/*
    ↓ proxies to 127.0.0.1:6109
...
Host Nginx (port 443, SSL + WebSocket Upgrade)
    ↓ rewrites /host1/websockify → /websockify
    ↓ proxies to 127.0.0.1:6109
Container Nginx (backend_host_1, port 80)
    ↓ proxies WebSocket to NoVNC
