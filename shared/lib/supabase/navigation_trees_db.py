...
def get_tree(tree_id: str, team_id: str) -> Optional[Dict]:
    """Legacy compatibility function - returns tree in old format using the view."""
    try:
        supabase = get_supabase()
        result = supabase.table('navigation_trees_legacy').select('*').eq('id', tree_id).eq('team_id', team_id).single().execute()
        ...
        print(f"[@db:navigation_trees:get_tree] Legacy compatibility error: {e}")
        return None

def save_tree(tree_data: Dict, team_id: str) -> str:
    """Legacy compatibility function - saves tree from old format."""
    print(f"[@db:navigation_trees:save_tree] WARNING: Using legacy save_tree function. Please migrate to new normalized API.")
    ...
    try:
        ...
        # Save tree metadata
        tree_result = save_tree_metadata(tree_metadata, team_id)
        if not tree_result['success']:
            return tree_result['error']
        
        tree_id = tree_result['tree']['id']
        
        # Extract and save nodes/edges from metadata
        ...
        for edge in edges:
            edge_data = {
                ...
                'final_wait_time': edge.get('data', {}).get('final_wait_time', 0)
            }
            edge_list.append(edge_data)
        
        # Save normalized data
        batch_result = save_tree_data(tree_id, node_list, edge_list, team_id)
        ...
        print(f"[@db:navigation_trees:save_tree] Legacy compatibility error: {e}")
        return str(e) 