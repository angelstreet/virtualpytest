...
def get_per_device_metrics_lightweight(devices: List) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        for device in devices:
            ...
            # Check FFmpeg status for this device
            ...
            if ffmpeg_status.get('recent_files', {}).get(capture_folder):
                ...
                if device_files.get('images', 0) > 0 or device_files.get('video_segments', 0) > 0:
                    ...
                    ffmpeg_device_status = 'stuck' if ffmpeg_processes_running else 'stopped'
            else:
                ffmpeg_device_status = 'stuck' if ffmpeg_processes_running else 'stopped'
            
            clear_process_cache_if_stuck(capture_folder, 'ffmpeg', ffmpeg_device_status)
            
            # Check Monitor status for this device
            ...
            if monitor_status.get('recent_files', {}).get(capture_folder):
                device_files = monitor_status['recent_files'][capture_folder]
                if device_files.get('images', 0) > 0:
                    ...
                    monitor_device_status = 'stuck' if monitor_process_running else 'stopped'
            else:
                monitor_device_status = 'stuck' if monitor_process_running else 'stopped'
            ...
            device_metric = {
                ...
                'ffmpeg_uptime_seconds': ffmpeg_uptime_seconds,
