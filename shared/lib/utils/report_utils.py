...
def record_script_step(team_id: str, script_name: str, step_number: int, step_name: str, 
                      step_description: str, success: bool, execution_time_ms: int,
                      screenshot_path: str = None, step_category: str = 'general',
                      error_message: str = None) -> bool:
    """
    Record a script execution step to the database with proper reporting.
    
    Args:
        team_id: Team ID for database context
        script_name: Name of the script (e.g., 'fullzap', 'goto_live')
        step_number: Sequential step number
        step_name: Short name for the step (e.g., 'Action: live_chup (iteration 1)')
        step_description: Detailed description
        success: Whether the step succeeded
        execution_time_ms: Execution time in milliseconds
        screenshot_path: Optional path to screenshot
        step_category: Category of step ('navigation', 'action', 'verification', etc.)
    ...
    """
    try:
        from shared.lib.supabase.execution_results_db import record_step_execution
        
        print(f"[@utils:report_utils:record_script_step] Recording {step_category} step {step_number}: {step_name}")
        ...
        print(f"{status_emoji} [{script_name}] Step {step_number} {'completed successfully' if success else 'failed'} in {execution_time_ms}ms")
        
        return True
        
    except Exception as e:
        print(f"[@utils:report_utils:record_script_step] ERROR: Failed to record step: {str(e)}")
        return False

def record_navigation_step(team_id: str, script_name: str, step_number: int, 
                          source_node: str, target_node: str, success: bool,
                          execution_time_ms: int, screenshot_path: str = None,
                          error_message: str = None) -> bool:
    """
    Record a navigation step with standardized naming.
    
    Args:
        team_id: Team ID for database context
        script_name: Name of the script
        step_number: Sequential step number
        source_node: Source node label
    ...
    """
    step_name = f"Navigation step {step_number}: {source_node} → {target_node}"
    step_description = f"Navigate from '{source_node}' to '{target_node}'"
    
    return record_script_step(
        team_id=team_id,
        script_name=script_name,
        step_number=step_number,
        step_name=step_name,
...
def record_action_step(team_id: str, script_name: str, step_number: int,
                      action_name: str, iteration: int, max_iterations: int,
                      success: bool, execution_time_ms: int, 
                      screenshot_path: str = None, error_message: str = None) -> bool:
    """
    Record an action execution step with standardized naming.
    
    Args:
        team_id: Team ID for database context
        script_name: Name of the script
        step_number: Sequential step number
        action_name: Name of the action (e.g., 'live_chup')
    ...
    Returns:
        Boolean indicating success
    """
    step_name = f"Action: {action_name} (iteration {iteration})"
    step_description = f"Execute {action_name} action - iteration {iteration}/{max_iterations}"
...
def record_verification_step(team_id: str, script_name: str, step_number: int,
                           verification_type: str, success: bool, execution_time_ms: int,
                           screenshot_path: str = None, error_message: str = None) -> bool:
    """
    Record a verification step with standardized naming.
    ...
    Returns:
        Boolean indicating success
    """
    step_name = f"Verification: {verification_type}"
    step_description = f"Perform {verification_type} verification"
