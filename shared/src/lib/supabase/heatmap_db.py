...
def save_heatmap_to_db(
    ...
    Note: id is auto-generated by Supabase (DEFAULT gen_random_uuid())
    job_id must be provided as UUID (no default in schema)
    """
    try:
        heatmap_data = {
            'team_id': team_id,
        ...
        print(f"[@db:heatmap:save_heatmap_to_db] Saving heatmap with job_id: {job_id}")
        
        supabase = get_supabase()
        result = supabase.table('heatmaps').insert(heatmap_data).execute()
        
        if result.data and len(result.data) > 0:
            heatmap_id = result.data[0]['id']
            print(f"[@db:heatmap:save_heatmap_to_db] Success: {heatmap_id}")
            return heatmap_id
        else:
            print(f"[@db:heatmap:save_heatmap_to_db] Failed - no data returned from insert")
            return None
            
    except Exception as e:
        print(f"[@db:heatmap:save_heatmap_to_db] Error: {str(e)}")
        import traceback
        print(f"[@db:heatmap:save_heatmap_to_db] Traceback: {traceback.format_exc()}")
        return None
