...
def get_all_interfaces_with_trees(team_id: str) -> Dict:
    """
    Get ALL user interfaces with their complete tree data in ONE call.
    Uses materialized view for instant reads of all tree data.
    ...
    try:
        ...
        for interface in interfaces:
            ...
            if root_tree:
                # Get full tree data using optimized function
                ...
                if tree_data['success']:
                    ...
                    # Only include interfaces that have actual nodes
                    if nodes_count > 0:
                        result.append({
                            'interface': interface,
                            'tree_data': tree_data
                        ...
                        print(f"[@db:navigation_trees:get_all_interfaces_with_trees] ✅ {interface['name']}: {nodes_count} nodes, {edges_count} edges")
                    else:
                        print(f"[@db:navigation_trees:get_all_interfaces_with_trees] ⚠️ {interface['name']}: No nodes - excluded")
        
        print(f"[@db:navigation_trees:get_all_interfaces_with_trees] Loaded {len(result)} interfaces with {total_nodes} total nodes, {total_edges} total edges")
        ...
        print(f"[@db:navigation_trees:get_all_interfaces_with_trees] Error: {e}")
        return {'success': False, 'error': str(e)}
