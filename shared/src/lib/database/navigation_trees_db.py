...
def get_full_tree(tree_id: str, team_id: str) -> Dict:
    """
    Get complete tree data (metadata + nodes + edges) using optimized database function.
    
    Performance: ~500ms for single query (replaces 3 separate queries at ~1400ms)
    """
    try:
        supabase = get_supabase()
        
        # Use Supabase function for optimized retrieval
        result = supabase.rpc(
        ...
        if result.data:
            ...
            print(f"[@db:navigation_trees:get_full_tree] Retrieved tree {tree_id} in single query")
