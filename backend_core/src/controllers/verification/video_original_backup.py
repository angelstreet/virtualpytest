...
JSON ONLY - NO OTHER TEXT"""
    ...
    def detect_motion_from_json(self, json_count: int = 5, strict_mode: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            # Read JSON files directly from capture path
            import os
            import glob
            import json
            from datetime import datetime, timedelta
            ...
            # Find recent JSON files in the capture path
            json_pattern = os.path.join(capture_path, "*.json")
            ...
            if not json_files:
                return {
                    ...
                    'details': [],
                    'strict_mode': strict_mode,
                    'message': f'No JSON analysis files found in {capture_path}'
                }
            
            # Sort by modification time (most recent first) and take the requested count
            json_files.sort(key=os.path.getmtime, reverse=True)
            recent_files = json_files[:json_count]
            
            print(f"VideoVerify[{self.device_name}]: Found {len(json_files)} JSON files, analyzing {len(recent_files)} recent files")
            ...
            for json_file in recent_files:
                try:
                    with open(json_file, 'r') as f:
                        ...
                        print(f"VideoVerify[{self.device_name}]: Loaded {os.path.basename(json_file)}")
                except Exception as e:
                    print(f"VideoVerify[{self.device_name}]: Failed to load {os.path.basename(json_file)}: {e}")
            ...
            if not analysis_data:
                return {
                    ...
                    'details': [],
                    'strict_mode': strict_mode,
                    'message': f'No valid JSON analysis data found in {len(recent_files)} files'
                }
            
            # Create data_result structure for analyze_motion_from_loaded_data
            data_result = {
                'success': True,
                'analysis_data': analysis_data
            }
            
            # Import shared analysis utility for motion analysis
            from shared.lib.utils.analysis_utils import analyze_motion_from_loaded_data
            ...
            print(f"VideoVerify[{self.device_name}]: Motion detection result: {result.get('message', 'Unknown')}")
            
            # Log verification for tracking
            self._log_verification("motion_from_json", f"count_{json_count}_strict_{strict_mode}", result['success'], {
                "json_count": json_count,
                "strict_mode": strict_mode,
                "total_analyzed": result['total_analyzed'],
                "blackscreen_count": result['blackscreen_count'],
