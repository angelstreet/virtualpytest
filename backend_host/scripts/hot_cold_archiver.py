...
def clean_old_metadata(capture_dir: str) -> int:
    """
    Clean old metadata files from /hot/metadata/ directory - keep only newest 100 files.
    
    Metadata files (JSON) are small but should be cleaned for consistency.
    ...
    if ram_mode:
        hot_dir = os.path.join(capture_dir, 'hot', 'metadata')
    else:
        hot_dir = os.path.join(capture_dir, 'metadata')
    
    if not os.path.isdir(hot_dir):
        return 0
    
    metadata_limit = HOT_LIMITS.get('metadata', 100)
    
    try:
        # Get all metadata files
        files = []
        for item in Path(hot_dir).glob('capture_*.json'):
            if item.is_file() and item.parent == Path(hot_dir):
                files.append(item)
        
        file_count = len(files)
        
        if file_count < metadata_limit:
            if file_count > 0:
                logger.debug(f"metadata: {file_count} files (within limit {metadata_limit})")
            return 0
        
        # Calculate how many to delete
        to_delete = file_count - metadata_limit
        
        # Sort by modification time (oldest first)
        files.sort(key=lambda f: f.stat().st_mtime)
        
        # Delete oldest metadata files
        deleted_count = 0
        for filepath in files[:to_delete]:
            try:
                os.remove(str(filepath))
                ...
                logger.error(f"Error deleting metadata {filepath}: {e}")
        
        logger.info(f"metadata: Deleted {deleted_count} old files ({file_count} → {file_count - deleted_count}, target: {metadata_limit})")
...
def process_capture_directory(capture_dir: str):
    ...
    3. Clean old metadata (keep newest 100 for consistency)
    ...
    # 3. Clean old metadata (keep newest 100 for consistency)
    deleted_metadata = clean_old_metadata(capture_dir)
    ...
    logger.info(f"✓ Completed {capture_dir} in {elapsed:.2f}s (deleted {deleted_captures} captures, {deleted_thumbnails} thumbnails, {deleted_metadata} metadata, archived {archived_segments} segments)")


def main_loop():
    """
    Main service loop - SEGMENTS-ONLY archival
    Archives only segments every 1min (captures rotate in RAM)
    """
    logger.info("=" * 60)
