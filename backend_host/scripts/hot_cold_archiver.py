...
#
# RAM Usage (HIGH QUALITY CAPTURES):
# - Segments: 150 × 45KB = 6.8MB (2.5min buffer, archive every 2min)
# - Captures: 700 × 60KB = 42MB (120s buffer at 5fps, HD captures at 1280x720 for best detection)
# - Thumbnails: 700 × 12KB = 8.4MB (Same rate as captures, 2min buffer)
# Total: ~57MB per device ✅ (28.5% of 200MB budget, high quality for detection)
#
HOT_LIMITS = {
    'segments': 150,      # 2.5min buffer → archive to hour folders
    'captures': 700,      # 120s buffer at 5fps (600) + 100 safety margin for v4l2
    'thumbnails': 700,    # 2min buffer at 5fps (same as captures)
...
def clean_old_thumbnails(capture_dir: str) -> int:
    """
    Clean old thumbnails from /hot/thumbnails/ directory - keep only newest 700 files.
    
    Thumbnails are generated by FFmpeg at same rate as captures (5fps v4l2, 2fps VNC).
    We keep a buffer (700 files = ~8.4MB, 2min buffer at 5fps) for recent incidents.
    ...
    # Keep only 700 thumbnails (FFmpeg-generated at 5fps, 2min buffer)
    thumbnail_limit = HOT_LIMITS.get('thumbnails', 700)
...
def process_capture_directory(capture_dir: str):
    ...
    2. Clean old thumbnails (keep newest 700, FFmpeg-generated at 5fps)
    ...
    # 2. Clean old thumbnails (keep newest 700 at 5fps = 2min buffer)
