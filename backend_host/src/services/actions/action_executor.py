...
class ActionExecutor:
    ...
    def _execute_single_action(self, action: Dict[str, Any], execution_order: int, action_number: int, action_category: str, team_id: str = None, context = None) -> Dict[str, Any]:
        ...
        # Write action metadata to frame JSON for automatic zap measurement
    ...
    def _write_action_to_frame_json(self, action: Dict[str, Any], action_completion_timestamp: float):
        """
                ...
                try:
                    with open(lock_path, 'w') as lock_file:
                        ...
                        try:
                            ...
                            print(f"[@action_executor:_write_action_to_frame_json] ✅ {best_match_file} | delta={int(min_delta*1000)}ms | ts={action_completion_timestamp} | params={action.get('params', {})}")
                        finally:
                    ...
                    print(f"[@action_executor:_write_action_to_frame_json] ❌ Failed to update: {best_match_file} | error: {e}")
            elif best_match_file:
                # Tolerance exceeded - detailed logging
                print(f"[@action_executor:_write_action_to_frame_json] ⚠️ No match within 1500ms | best: {best_match_file} | delta: {int(min_delta*1000)}ms")
            else:
                # No files found - brief logging
                print(f"[@action_executor:_write_action_to_frame_json] ⚠️ No matching frames found")
        
        except Exception as e:
            # Non-blocking - log error but don't fail action execution
            print(f"[@action_executor:_write_action_to_frame_json] Error writing to frame JSON: {e}")
